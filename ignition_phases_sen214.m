%Ana Sanjuan 2016 based on Gustavo Deco ignition 
%output
...
clear
clc
load('F:\NMS\ET\fMRI\Dyn\ET.mat')
timeseries{1,1}=CNT_Temp;
timeseries{1,2}=ET_Pre_Temp;
timeseries{1,3}=ET_Post_Temp;
%% %% Ignition Measures
...
%   stdignitionon=mean(stdignitionon2);
  ...
  cd ('F:\CSG\fMRI\FrontoParietalDOC\GT_DFC\IntensicIgnition')

  save (sprintf('%dTR_SS_Ignition_CASE%d_Phases', nTRs-1, CASE), 'mevGL', 'stdevGL', 'mevokedinteg',...
      'mevokedintegSS', 'stdevokedintegSS', 'mignitionAN', 'stdignitionAN');
      % figure(cond)
      % mevokedinteg2OLD(seed)=max(mean(squeeze(IntegStim2(seed,:,1:nevents2(seed))),2));
      mevokedinteg2(seed) = mean(max(squeeze(IntegStim2(seed,:,1:nevents2(seed)))));
      stdevokedinteg2(seed) = std(max(squeeze(IntegStim2(seed,:,1:nevents2(seed)))));
 clearvars 'mevGL' 'stdevGL' 'mevokedinteg' 'mevokedintegSS' 'stdevokedintegSS' 'mignitionAN' 'stdignitionAN'
cd ..
end
